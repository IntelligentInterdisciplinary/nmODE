...
logging.basicConfig(filename='0812_Res18_4096_15e-1_20_1_edit.log', level=logging.INFO)
...
trainset    = torchvision.datasets.ImageFolder(root='./data/CUB_200_2011/CUB_200_2011/dataset/train', transform=transform_train)
trainloader = torch.utils.data.DataLoader(trainset, batch_size=32, shuffle=True, num_workers=12, drop_last = True)

testset = torchvision.datasets.ImageFolder(root='./data/CUB_200_2011/CUB_200_2011/dataset/test', transform=transform_test)
testloader = torch.utils.data.DataLoader(testset, batch_size=32, shuffle=True, num_workers=12)
...
class nmODEblock(torch.nn.Module):
    def __init__(self, feature_size=512,classes_num=200):
        ...
        self.LP1 = torch.nn.Linear(feature_size,4096)
        self.LP2 = torch.nn.Linear(feature_size,4096)
        self.LP3 = torch.nn.Linear(4096,classes_num)
    ...
    def forward(self,x):
        ...
        y = odeint(self.neuralODE, y, self.t.to(y.device))[-1]
...
class model_bn(nn.Module):
    def __init__(self, feature_size=512,classes_num=200):
        ...
        self.classifier = nn.Sequential(
            ...
            nn.ELU(inplace=True),
            #nn.Dropout(0.5),
            nmODEblock(feature_size, classes_num),
        )
